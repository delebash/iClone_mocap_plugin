...
def gen_mocap_pt_arr(json_file):
    '''
...
def update_new_pose(pose_pts_arr, mocap_hand_arr):
    '''
    generate new pose
    :param pose_pts_arr:
    :param mocap_pts_arr:
    :return:
    '''
    # mocap_pts_arr = mocap_pts_arr['points']
    # finger = mocap_pts_arr['finger']
    ...
    # group_num = len(mocap_pts_arr) / 5  # number of detected fingers
    pose_pts_arr = np.array(pose_pts_arr)
    joints = []
    for index in range(len(mocap_hand_arr)):
        fingerarry = mocap_hand_arr[index]
        finger = fingerarry['finger']
        fingerjoints = fingerarry['points']

        for i in  range(len(fingerjoints)):
            joints.append(fingerjoints[i])
        # for joints in range(len(mocap_hand_arr[index])):
        #     pose_arr_start_position = 0
        #     finger = joints['finger']
        #     mocap_pts_arr = joints['points']
        ...
        poses = -1 * poses
        # pose_arr_start_position = 40
        # pose_pts_arr[pose_arr_start_position] = [1, 1, 1, 0, -30, 0]
        # test =  pose_pts_arr[pose_arr_start_position]
        # pose_arr_start_position = 40
        # np.put(pose_pts_arr, 40, [1,2])
        # # pose_pts_arr[pose_arr_start_position] = [1, 2]
        pose_arr_start_position = 0
        if finger == 'thumb':
            pose_arr_start_position = 40
        ...
        elif finger == 'indexFinger':
            pose_arr_start_position = 43
        elif finger == 'middleFinger':
            pose_arr_start_position = 47
        elif finger == 'ringFinger':
            pose_arr_start_position = 51
        elif finger == 'pinky':
            pose_arr_start_position = 55
        pose_pts_arr[pose_arr_start_position] = [poses[0], poses[1], poses[2], 0, 0, 0]

    return pose_pts_arr


def json2iclon(tpose, json_file):
    '''
    ...
    mocap_pts_arr = gen_mocap_pt_arr(json_file)
