...
def update_new_pose(pose_pts_arr, mocap_pts_arr):
    ...
    # left_hand_thumb_1 = mocap_pts_arr[40] + mocap_pts_arr[41] + mocap_pts_arr[42]
    # left_in_hand_index = mocap_pts_arr[43] + mocap_pts_arr[44] + mocap_pts_arr[45] + mocap_pts_arr[46]
    # left_in_hand_middle = mocap_pts_arr[47] + mocap_pts_arr[48] + mocap_pts_arr[49] + mocap_pts_arr[50]
    # left_in_hand_ring = mocap_pts_arr[51] + mocap_pts_arr[52] + mocap_pts_arr[53] + mocap_pts_arr[54]
    # left_hand_pinky = mocap_pts_arr[55] + mocap_pts_arr[56] + mocap_pts_arr[57] + mocap_pts_arr[58]

    # pose_pts_arr = np.array(pose_pts_arr)
    #
    # for obj in mocap_pts_arr:
    #     finger = obj['finger']
    #     finger_pts = obj['points']
    #
    #     vs = get_vecs(finger_pts)
    #     poses = get_pose(vs)
    #     #
    #     # left hand
    #     poses = np.array(poses)
    ...
    #
    #     if finger == 'thumb':
    #         pose_pts_arr[0] = finger_pts[0] + np.array([0, 0, 0, 0, poses[0], 0])
    #         pose_pts_arr[1] = finger_pts[1] + np.array([0, 0, 0, 0, poses[1], 0])
    #         pose_pts_arr[2] = finger_pts[2] + np.array([0, 0, 0, 0, poses[2], 0])
    #     else:
    #         pose_pts_arr[0] = finger_pts[0] + np.array([0, 0, 0, 0, poses[0], 0])
    #         pose_pts_arr[2] = finger_pts[2] + np.array([0, 0, 0, 0, poses[1], 0])
    #         pose_pts_arr[3] = finger_pts[3] + np.array([0, 0, 0, 0, poses[2], 0])
    #
